...
#extedning Simple Kmeans implementation
class SimpleKMeans:
...
#Normal mhubert has faiss index we follow a Kmeans clustering approach
class KMeansIndex:
...
#Loading mhubert without with reference to the index path 
class Hubert2Unit(torch.nn.Module):
    ...
    def forward(self, wav, wav_lengths, do_normalize=True):
        with torch.no_grad():
            ...
            ).hidden_states[9]  # specify the layer of extraction (6th layer for dsu in normal hubert)
            ...
            if not self.index.fitted:
                print("Fitting KMeans model to hidden states...")
...
def process_wav_directory(input_dir, output_file):
    ...
    with open(output_file, 'w') as f:
        for wav_file in tqdm(wav_files, desc="Processing WAV files"):
            ...
            f.write(f"{wav_file.replace('.wav','')}|{dsu_string}\n")
    
    print(f"Processed {len(wav_files)} WAV files. Results saved in {output_file}")
    
lst = ['Telugu'] #Example language stated
for i in lst:
    input_directory = #input directory MUST-C preferred
    output_directory = #ouput directory specified
